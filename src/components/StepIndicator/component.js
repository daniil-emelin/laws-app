...
class StepIndicator extends PureComponent {
  ...
  renderStepIndicator = () => {
    let steps = [];
    const { labels, stepCount, direction, indexQ } = this.props;
    ...
    renderStep = (position) => {
      const { currentPosition, stepCount, direction, indexQ } = this.props;
      let stepStyle;
      ...
      switch (this.getStepStatus(position)) {
        ...
        case STEP_STATUS.FINISHED:{
          stepStyle = {
            backgroundColor:this.props.answers.length ? this.props.answers[position].right ? 'rgb(61,184,120)' : 'rgb(244, 111, 95)':'rgb(244, 111, 95)'/* this.customStyles.stepIndicatorFinishedColor */,
